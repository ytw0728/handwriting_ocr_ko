...
class MultiHeadAttention:
    ...
    def get_weights(self) -> dict:
        ...
        weights = {}
        for i in range(self.n_heads):
            weights[f"q_mapping_{i}"] = self.q_mappings[i].get_weights()
            weights[f"k_mapping_{i}"] = self.k_mappings[i].get_weights()
            weights[f"v_mapping_{i}"] = self.v_mappings[i].get_weights()
        return weights
    def set_weights(self, weights: dict) -> None:
        ...
        """
        for i in range(self.n_heads):
            self.q_mappings[i].set_weights(weights[f"q_mapping_{i}"])
            self.k_mappings[i].set_weights(weights[f"k_mapping_{i}"])
            self.v_mappings[i].set_weights(weights[f"v_mapping_{i}"])