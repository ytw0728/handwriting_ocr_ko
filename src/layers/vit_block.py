...
class ViTBlock:
    ...
    def get_weights(self) -> dict:
        """Get weights of the block.

        Returns:
            dictionary containing weights of the block.
        """
        weights = {
            "layer_norm_1": self.layer_norm_1.get_weights(),
            "mha": self.mha.get_weights(),
            "layer_norm_2": self.layer_norm_2.get_weights(),
            "mlp_1": self.mlp_1.get_weights(),
            "mlp_2": self.mlp_2.get_weights(),
        }
        return weights

    def set_weights(self, weights: dict) -> None:
        """Set weights of the block.

        Args:
            weights: dictionary containing weights of the block.
        """
        self.layer_norm_1.set_weights(weights["layer_norm_1"])
        self.mha.set_weights(weights["mha"])
        self.layer_norm_2.set_weights(weights["layer_norm_2"])
        self.mlp_1.set_weights(weights["mlp_1"])
        self.mlp_2.set_weights(weights["mlp_2"])