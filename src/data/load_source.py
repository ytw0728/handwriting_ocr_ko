...
def load_source(split: str, label: Label, output_size:int, max_answer_length:int, resize=(256, 128), include_hanja=False, max_length=40) -> tuple[np.ndarray[np.float64, np.dtype[np.float64]], np.ndarray[dict], np.ndarray[np.ndarray[int]]]:
    ...
    metas.extend([{'pen_color':0, 'pen_type':0} for _ in range(max_length - len(metas))])
    answers.extend(
        np.array([
