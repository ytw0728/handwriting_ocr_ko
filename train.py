...
if __name__ == "__main__":
    ...
    train_loader = StreamingDataLoader(label_files[0:max_file_size], batch_size=batch_size, prefetch_batches=2, output_size=output_size)
    ...
    for epoch in outer_tqdm:
        ...
        try:
            inner_tqdm = tqdm(train_loader, desc=f"Epoch {epoch+1}/{max_epochs}", leave=False)
            for imgs_batch, answer_batch, answer_length_batch in inner_tqdm:
                # for imgs_batch, metas_batch, answer_batch in tqdm(train_loader):
        ...
        network.save_weights(f"vit_weights-({max_file_size},{max_epochs})-{(epoch+1)}.pkl")
    network.save_weights(f"vit_weights-({max_file_size},{max_epochs})-final.pkl")
    print("Training Done")